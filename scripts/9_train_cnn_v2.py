...
def build_cnn_model(num_classes, structured_feature_count):
    ...
    x = layers.Embedding(input_dim=3000, output_dim=64, name="char_embedding")(x)
    x = layers.Conv1D(filters=128, kernel_size=5, activation="relu", name="conv_5")(x)
    x = layers.MaxPooling1D(pool_size=2)(x)
    x = layers.Conv1D(filters=128, kernel_size=3, activation="relu", name="conv_3")(x)
...
def main():
    ...
    model, vectorizer = build_cnn_model(
    ...
    vectorizer.adapt(
    tf.data.Dataset.from_tensor_slices(X_train_text).batch(128)
)
    ...
    weights_path = MODEL_DIR / "cnn_multilingual_features_model.weights.h5"
    model.save_weights(weights_path)
    print(f"\nCNN model weights saved to: {weights_path}")

    vocab_path = MODEL_DIR / "cnn_character_vectorizer_vocabulary.txt"
    ...
    preprocessing_path = MODEL_DIR / "cnn_preprocessing_objects.pkl"
    joblib.dump(
        {
    ...
    results_path = RESULTS_DIR / "cnn_v2_results.txt"
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("CNN Benchmark - Multilingual V2 Dataset\n")
        f.write("=" * 60 + "\n\n")
